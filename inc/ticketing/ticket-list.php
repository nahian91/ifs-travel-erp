 ...
 * Features: Live Summary Analytics, Dual PNR Badges, Dynamic Search, Batch Filters & Action Controls
 */
function ifs_terp_ticket_list_page() {
    ...
    <div class="ifs-ticket-list-workspace">
        ...
        <div class="ifs-list-metric-ribbon">
            <div class="ifs-metric-chip">
                ...
                <div>
                    <span class="chip-label"><?php esc_html_e( 'Total Issued Tickets', 'ifs-travel-erp' ); ?></span>
                    <strong class="chip-val"><?php echo esc_html( number_format_i18n( $total_tickets ) ); ?></strong>
                </div>
            </div>
            <div class="ifs-metric-chip">
                <div class="chip-icon bg-indigo"><span class="dashicons dashicons-money-alt"></span></div>
                <div>
                    <span class="chip-label"><?php esc_html_e( 'Gross Air Turnover', 'ifs-travel-erp' ); ?></span>
                    <strong class="chip-val color-blue">৳<?php echo esc_html( number_format( $total_sales, 2 ) ); ?></strong>
                </div>
            </div>
            <div class="ifs-metric-chip">
                <div class="chip-icon bg-emerald"><span class="dashicons dashicons-chart-line"></span></div>
                <div>
                    <span class="chip-label"><?php esc_html_e( 'Net Ticket Profit', 'ifs-travel-erp' ); ?></span>
                    <strong class="chip-val color-emerald">৳<?php echo esc_html( number_format( $total_profit, 2 ) ); ?></strong>
                </div>
            </div>
            <div class="ifs-metric-chip">
                <div class="chip-icon bg-amber"><span class="dashicons dashicons-image-rotate"></span></div>
                <div>
                    <span class="chip-label"><?php esc_html_e( 'Refund / Void Files', 'ifs-travel-erp' ); ?></span>
                    <strong class="chip-val color-amber"><?php echo esc_html( number_format_i18n( $refund_count ) ); ?></strong>
                </div>
            </div>
        </div>
        ...
        <div class="ifs-table-card">
            <div class="ifs-table-top-bar">
                <div class="ifs-table-title-group">
                    <h3 class="ifs-table-heading"><span class="dashicons dashicons-airplane"></span> <?php esc_html_e( 'Air Tickets Ledger & Inventory', 'ifs-travel-erp' ); ?></h3>
                    <p class="ifs-table-caption"><?php esc_html_e( 'GDS issued flight manifests, traveler routing details, and status management', 'ifs-travel-erp' ); ?></p>
                </div>
                <div class="ifs-table-btn-group">
                    <a href="<?php echo esc_url( $base_url . '&sub=add' ); ?>" class="ifs-btn-primary">
                        <span class="dashicons dashicons-plus-alt2"></span> <?php esc_html_e( 'Issue New Air Ticket', 'ifs-travel-erp' ); ?>
                    </a>
                </div>
            </div>
                ...
                <table class="ifs-pro-datatable" id="ifsTicketTable">
                    <thead>
                        <tr>
                            ...
                            <th><?php esc_html_e( 'GDS / Airline PNR', 'ifs-travel-erp' ); ?></th>
                            ...
                            <th style="text-align: right; width: 160px;"><?php esc_html_e( 'Quick Actions', 'ifs-travel-erp' ); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if ( $tickets ) : foreach ( $tickets as $row ) : 
                            ...
                            if ( 'refunded' === $status_lower )  $status_class = 'status-refunded';
                            elseif ( 'void' === $status_lower )  $status_class = 'status-void';
                            elseif ( 'reissued' === $status_lower ) $status_class = 'status-reissued';
                            ...
                            <tr>
                                    ...
                                    <div class="ifs-pnr-cell">
                                        <div style="display: flex; align-items: center; gap: 4px;">
                                            ...
                                            <?php if ( ! empty( $row->airline_pnr ) ) : ?>
                                                <span class="ifs-airpnr-pill font-mono" title="<?php esc_attr_e( 'Airline PNR', 'ifs-travel-erp' ); ?>"><?php echo esc_html( $row->airline_pnr ); ?></span>
                                            <?php endif; ?>
                                        </div>
                                        <span class="ifs-gds-source"><?php echo esc_html( $gds_label ); ?></span>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td style="text-align: right;">
                                    <div class="ifs-action-pills">
                                        <a href="<?php echo esc_url( $base_url . '&sub=view&id=' . $row->id ); ?>" class="ifs-action-pill view" title="<?php esc_attr_e( 'View E-Ticket Voucher', 'ifs-travel-erp' ); ?>">
                                            <span class="dashicons dashicons-visibility"></span> <?php esc_html_e( 'View', 'ifs-travel-erp' ); ?>
                                        </a>
                                        <a href="<?php echo esc_url( $base_url . '&sub=edit&id=' . $row->id ); ?>" class="ifs-action-pill edit" title="<?php esc_attr_e( 'Edit Ticket Record', 'ifs-travel-erp' ); ?>">
                                            <span class="dashicons dashicons-edit"></span> <?php esc_html_e( 'Edit', 'ifs-travel-erp' ); ?>
                                        </a>
                                        <a href="<?php echo esc_url( wp_nonce_url( $base_url . '&sub=delete&id=' . $row->id, 'delete_ticket_' . $row->id ) ); ?>" 
                                           class="ifs-action-pill delete" 
                                           onclick="return confirm('<?php esc_attr_e( 'Are you sure you want to permanently delete this ticket record?', 'ifs-travel-erp' ); ?>');" 
                            ...
                            <tr>
                                <td colspan="7" class="ifs-empty-table">
                                    <div class="ifs-empty-state">
                                        ...
                                        <p><?php esc_html_e( 'Issue your first GDS flight ticket to initialize passenger manifest tracking.', 'ifs-travel-erp' ); ?></p>
                                        <a href="<?php echo esc_url( $base_url . '&sub=add' ); ?>" class="ifs-btn-primary" style="margin-top: 10px;">
                                            <span class="dashicons dashicons-plus-alt2"></span> <?php esc_html_e( 'Issue Air Ticket', 'ifs-travel-erp' ); ?>
                                        </a>
                                    </div>
                                </td>
                            </tr>
    ...
    <style>
        .ifs-ticket-list-workspace { max-width: 1440px; margin: 0 auto; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; color: #0f172a; }
        ...
        .ifs-btn-primary { background: linear-gradient(135deg, #003376 0%, #0284c7 100%); color: #ffffff !important; border: none; padding: 9px 18px; border-radius: 8px; font-size: 13px; font-weight: 700; text-decoration: none; display: inline-flex; align-items: center; gap: 6px; box-shadow: 0 4px 12px rgba(0, 51, 118, 0.2); transition: all 0.2s ease; }
        .ifs-btn-primary:hover { transform: translateY(-1px); box-shadow: 0 6px 16px rgba(0, 51, 118, 0.3); }
    ...
    jQuery(document).ready(function($) {
        if ($.fn.DataTable) {
            $('#ifsTicketTable').DataTable({
                ...
                "language": {
                    "search": "_INPUT_",
                    "searchPlaceholder": "Search PNR, Passenger, Status...",
