...
function ifs_terp_ticket_delete_handler() {
    ...
    if ( function_exists( 'ifs_terp_has_access' ) ) {
        if ( ! ifs_terp_has_access( array( 'admin_manager', 'ticketing_staff' ) ) && ! current_user_can( 'manage_options' ) ) {
    ...
    if ( ! wp_verify_nonce( $nonce, 'delete_ticket_' . $id ) ) {
