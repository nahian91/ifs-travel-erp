...
/**
 * Enterprise Ultra-Modern Segmented Sub-Navigation for Air Ticketing Module
 * @param string $active_tab The currently active sub-action
 */
function ifs_terp_ticketing_render_tabs( $active_tab = 'list' ) {
    global $wpdb;
    ...
    $today_tickets = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(id) FROM $table_tickets WHERE created_at BETWEEN %s AND %s AND status != 'Void'", $today_start, $today_end ) );
    $today_profit  = (float) $wpdb->get_var( $wpdb->prepare( "SELECT SUM(profit) FROM $table_tickets WHERE created_at BETWEEN %s AND %s AND status != 'Void'", $today_start, $today_end ) );
    ?>
            ...
            <nav class="ifs-pro-nav-bar">
                <a href="<?php echo esc_url( $base_url . '&sub=list' ); ?>" 
                   class="ifs-pro-nav-btn <?php echo ( $active_tab === 'list' ) ? 'active-tab' : ''; ?>">
                    ...
                    <span class="ifs-btn-label">All Issued Tickets</span>
                    <span class="ifs-pro-counter"><?php echo $total_tickets; ?></span>
                </a>

                <a href="<?php echo esc_url( $base_url . '&sub=add' ); ?>" 
                   class="ifs-pro-nav-btn <?php echo ( $active_tab === 'add' ) ? 'active-tab' : ''; ?>">
                    <span class="dashicons dashicons-plus-alt2"></span>
                    <span class="ifs-btn-label">Issue New Ticket</span>
                </a>

                <?php if ( $active_tab === 'edit' ) : ?>
                    <div class="ifs-pro-nav-btn state-active">
                        <span class="dashicons dashicons-edit"></span>
                        <span class="ifs-btn-label">Editing Ticket</span>
                ...
                <?php if ( $active_tab === 'view' ) : ?>
                    <div class="ifs-pro-nav-btn state-active">
                        ...
                        <span class="ifs-btn-label">Ticket Overview</span>
                        <span class="ifs-active-bubble"></span>
                    </div>
                <?php endif; ?>
            </nav>
