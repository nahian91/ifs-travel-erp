...
function ifs_terp_settings_hajj_pkg_list_panel() {
    global $wpdb;
    $table_pkgs = $wpdb->prefix . 'iterp_hajj_packages';
    $base_url   = admin_url( 'admin.php?page=ifs_travel_erp&tab=settings&sub=hajj_pkg' );
    $message    = '';

    // Handle Delete Action with Nonce Verification & Role Capability Check
    if ( isset( $_GET['action_sub'] ) && 'delete' === sanitize_key( wp_unslash( $_GET['action_sub'] ) ) && isset( $_GET['id'] ) ) {
        ...
        check_admin_referer( 'delete_hajj_pkg_' . $del_id );
        ...
        $redirect_url = add_query_arg( array( 'page' => 'ifs_travel_erp', 'tab' => 'settings', 'sub' => 'hajj_pkg', 'msg' => 'deleted' ), admin_url( 'admin.php' ) );
        if ( ! headers_sent() ) {
    ...
    if ( isset( $_GET['msg'] ) && 'deleted' === sanitize_key( wp_unslash( $_GET['msg'] ) ) ) {
        $message = '<div class="ifs-toast success"><span class="dashicons dashicons-yes-alt"></span> ' . esc_html__( 'Package template deleted successfully.', 'ifs-travel-erp' ) . '</div>';
    }

    $all_pkgs = $wpdb->get_results( "SELECT * FROM {$table_pkgs} ORDER BY id DESC" );
    $total_capacity = 0;
    if ( ! empty( $all_pkgs ) ) {
        foreach ( $all_pkgs as $p ) {
            $total_capacity += intval( $p->capacity ?? 0 );
        }
    }
    ?>
    <style>
        ...
        .dataTables_wrapper .dataTables_paginate .paginate_button.current {
            background: #047857 !important; border-color: #047857 !important; color: #ffffff !important;
        }
    ...
    <div class="wrap" style="max-width: 1400px; margin: 0 auto;">
        ...
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin-bottom: 20px;">
            <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px; display: flex; align-items: center; gap: 14px;">
                <span class="dashicons dashicons-palmtree" style="font-size: 28px; width: 28px; height: 28px; color: #047857;"></span>
                <div>
                    <span style="font-size: 11px; font-weight: 700; color: #64748b; text-transform: uppercase;"><?php esc_html_e( 'Total Packages', 'ifs-travel-erp' ); ?></span>
                    <h3 style="margin: 2px 0 0 0; font-size: 20px; font-weight: 900; color: #0f172a;"><?php echo esc_html( count( (array) $all_pkgs ) ); ?></h3>
                </div>
            </div>
            <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px; display: flex; align-items: center; gap: 14px;">
                <span class="dashicons dashicons-groups" style="font-size: 28px; width: 28px; height: 28px; color: #0284c7;"></span>
                <div>
                    <span style="font-size: 11px; font-weight: 700; color: #64748b; text-transform: uppercase;"><?php esc_html_e( 'Pilgrim Capacity', 'ifs-travel-erp' ); ?></span>
                    <h3 style="margin: 2px 0 0 0; font-size: 20px; font-weight: 900; color: #0f172a;"><?php echo esc_html( number_format( $total_capacity ) ); ?></h3>
                </div>
            </div>
            <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px; display: flex; align-items: center; gap: 14px;">
                <span class="dashicons dashicons-money-alt" style="font-size: 28px; width: 28px; height: 28px; color: #d97706;"></span>
                <div>
                    <span style="font-size: 11px; font-weight: 700; color: #64748b; text-transform: uppercase;"><?php esc_html_e( 'Pricing Structure', 'ifs-travel-erp' ); ?></span>
                    <h3 style="margin: 2px 0 0 0; font-size: 20px; font-weight: 900; color: #059669;"><?php esc_html_e( 'BDT & SAR Active', 'ifs-travel-erp' ); ?></h3>
                </div>
            </div>
        </div>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; border-bottom: 1px solid #f1f5f9; padding-bottom: 14px;">
                <div>
                    <h3 style="margin: 0; font-size: 16px; font-weight: 800; color: #0f172a; display: flex; align-items: center; gap: 8px;">
                        <span class="dashicons dashicons-list-view" style="color: #047857;"></span>
                        <?php esc_html_e( 'Configured Hajj & Umrah Packages Directory', 'ifs-travel-erp' ); ?>
                    </h3>
                    <p style="margin: 2px 0 0 0; font-size: 12.5px; color: #64748b;"><?php esc_html_e( 'Manage accommodation nights, quad/triple pricing, and airline allocations', 'ifs-travel-erp' ); ?></p>
                </div>
                <a href="<?php echo esc_url( add_query_arg( 'action_sub', 'add', $base_url ) ); ?>" style="background: #047857; color: #ffffff; padding: 8px 16px; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 13px; display: inline-flex; align-items: center; gap: 6px;">
                    <span class="dashicons dashicons-plus-alt2"></span> <?php esc_html_e( 'Add Package Template', 'ifs-travel-erp' ); ?>
                </a>
            </div>

            <table id="ifsHajjTable" class="wp-list-table widefat fixed" style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="background: #f8fafc; text-align: left;">
                        ...
                        <th style="padding: 12px 14px; font-size: 11px; text-transform: uppercase; color: #475569;"><?php esc_html_e( 'Type', 'ifs-travel-erp' ); ?></th>
                        <th style="padding: 12px 14px; font-size: 11px; text-transform: uppercase; color: #475569;"><?php esc_html_e( 'Duration', 'ifs-travel-erp' ); ?></th>
                ...
                <tbody>
                    <?php if ( ! empty( $all_pkgs ) ) : foreach ( $all_pkgs as $pkg ) : ?>
                        <tr>
                            <td style="padding: 12px 14px;">
                                <strong style="color: #0f172a; font-size: 13.5px;"><?php echo esc_html( $pkg->package_name ); ?></strong>
                                <?php if ( ! empty( $pkg->airline_name ) ) : ?>
                                    <div style="font-size: 11px; color: #64748b; margin-top: 2px;">
                                        <span class="dashicons dashicons-airplane" style="font-size: 13px; width: 13px; height: 13px; vertical-align: middle;"></span>
                                        <?php echo esc_html( $pkg->airline_name ); ?>
                                    </div>
                                <?php endif; ?>
                            </td>
                            <td style="padding: 12px 14px;">
                                <span style="background: <?php echo ( (string) $pkg->package_type === 'Hajj' ) ? '#fef3c7' : '#ecfdf5'; ?>; color: <?php echo ( (string) $pkg->package_type === 'Hajj' ) ? '#92400e' : '#047857'; ?>; padding: 3px 8px; border-radius: 4px; font-size: 11px; font-weight: 800;">
                                    <?php echo esc_html( $pkg->package_type ); ?>
                                </span>
                            </td>
                            <td style="padding: 12px 14px; font-family: monospace; color: #64748b;">
                                <?php echo intval( $pkg->total_days ); ?> <?php esc_html_e( 'Days', 'ifs-travel-erp' ); ?>
                                <div style="font-size: 10.5px; color: #94a3b8;"><?php echo intval( $pkg->nights_makkah ?? 0 ); ?>M / <?php echo intval( $pkg->nights_madinah ?? 0 ); ?>N</div>
                            </td>
                            <td style="padding: 12px 14px; text-align: right;">
                                <div style="display: flex; gap: 6px; justify-content: flex-end; align-items: center;">
                                    <!-- View Button with Icon & Text -->
                                    <a href="<?php echo esc_url( add_query_arg( array( 'action_sub' => 'view', 'id' => $pkg->id ), $base_url ) ); ?>" 
                                    ...
                                    <a href="<?php echo esc_url( add_query_arg( array( 'action_sub' => 'edit', 'id' => $pkg->id ), $base_url ) ); ?>" 
                                       ...
                                       title="<?php esc_attr_e( 'Edit Package', 'ifs-travel-erp' ); ?>">
                                    ...
                                    <a href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'action_sub' => 'delete', 'id' => $pkg->id ), $base_url ), 'delete_hajj_pkg_' . $pkg->id ) ); ?>" 
                                       ...
                                       title="<?php esc_attr_e( 'Delete Package', 'ifs-travel-erp' ); ?>">
                    ...
                    <?php endforeach; else : ?>
                        <tr><td colspan="4" style="text-align: center; padding: 40px; color: #94a3b8;"><?php esc_html_e( 'No Hajj/Umrah packages configured yet.', 'ifs-travel-erp' ); ?></td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
