...
if ( ! function_exists( 'ifs_terp_agent_render_tabs' ) ) {
    /**
     * Sub-Navigation Bar for B2B Sub-Agents Module
     * 
     * @param string $active_tab The currently active sub-action (list, add, edit, ledger)
     */
    function ifs_terp_agent_render_tabs( $active_tab = 'list' ) {
        ...
        $nav_items = array(
            'list' => array(
                'label' => __( 'All Agents', 'ifs-travel-erp' ),
                'icon'  => 'dashicons-networking',
                'url'   => add_query_arg( 'sub', 'list', $base_url ),
                'badge' => $total_agents,
            ),
            'add'  => array(
                'label' => __( 'Add Agent', 'ifs-travel-erp' ),
                'icon'  => 'dashicons-plus-alt2',
                'url'   => add_query_arg( 'sub', 'add', $base_url ),
            ),
        );

        // Contextual states for editing or viewing ledger
        if ( 'edit' === $active_tab ) {
            $nav_items['edit'] = array(
                'label' => __( 'Edit Agency', 'ifs-travel-erp' ),
                'icon'  => 'dashicons-edit',
                'url'   => '#',
            );
        } elseif ( 'ledger' === $active_tab ) {
            $nav_items['ledger'] = array(
                'label' => __( 'Agent Ledger', 'ifs-travel-erp' ),
        ...
        <div class="ifs-module-header">
            ...
            <div class="ifs-module-title-wrap">
                ...
                <div>
                    <h2 class="ifs-module-title"><?php esc_html_e( 'Agents', 'ifs-travel-erp' ); ?></h2>
                    <p class="ifs-module-subtitle"><?php esc_html_e( 'Manage B2B partners, credit limits, account balances, and ledgers', 'ifs-travel-erp' ); ?></p>
                </div>
            </div>
