...
if ( ! function_exists( 'ifs_terp_staff_render_tabs' ) ) {
    /**
     * Sub-Navigation Bar for Staff / HR Module
     * 
     * @param string $active_tab The currently active sub-action (list, add, edit)
     */
    function ifs_terp_staff_render_tabs( $active_tab = 'list' ) {
        $base_url = admin_url( 'admin.php?page=ifs_travel_erp&tab=staff' ); 

        // Quick count of users for the badge
        $user_count_data = count_users();
        $total_staff     = (int) ( $user_count_data['total_users'] ?? 0 );

        // Tab Definitions
        $nav_items = array(
            'list' => array(
                'label' => __( 'All Team', 'ifs-travel-erp' ),
                'icon'  => 'dashicons-groups',
                'url'   => add_query_arg( 'sub', 'list', $base_url ),
                'badge' => $total_staff,
            ),
            'add'  => array(
                'label' => __( 'Add Member', 'ifs-travel-erp' ),
                'icon'  => 'dashicons-plus-alt2',
                'url'   => add_query_arg( 'sub', 'add', $base_url ),
            ),
        );

        // Contextual state for editing staff credentials
        if ( 'edit' === $active_tab ) {
            $nav_items['edit'] = array(
                'label' => __( 'Edit Member', 'ifs-travel-erp' ),
                'icon'  => 'dashicons-edit',
                'url'   => '#',
            );
        }
        ...
        <div class="ifs-module-header">
            ...
            <div class="ifs-module-title-wrap">
                ...
                <div>
                    <h2 class="ifs-module-title"><?php esc_html_e( 'Team', 'ifs-travel-erp' ); ?></h2>
                    <p class="ifs-module-subtitle"><?php esc_html_e( 'Manage employee accounts, role permissions, and access controls', 'ifs-travel-erp' ); ?></p>
                </div>
            </div>
